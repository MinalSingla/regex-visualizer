...
// ════════════════════════════════════════════════════════════
// §1  REGEX PARSER  →  postfix via Shunting-Yard
// ════════════════════════════════════════════════════════════
// normalizeUnion: treat '+' as union when used as binary infix (a+b),
// keep '+' as Kleene-plus when used as unary postfix (a+, (ab)+).
// This lets the user type '+' for union; '|' also works as alias.
function normalizeUnion(re) {
  var out = '';
  for (var i = 0; i < re.length; i++) {
    var c = re[i];
    if (c === '\\') {
      // Escaped character — pass through as-is and skip next char
      out += c + (re[i + 1] || '');
      i++;
    } else if (c === '+') {
      var nx = re[i + 1];
      // Kleene-plus: nothing follows, or followed by an operator / close-paren
      var isKleene = !nx || nx === ')' || nx === '*' || nx === '+' || nx === '?' || nx === '|';
      out += isKleene ? '+' : '|';  // remap binary + → internal | (union)
    } else {
      out += c;
    }
  }
...
function insertConcat(re) {
  ...
  while (i < re.length) {
    var c = re[i];
    if (c === '\\') {
      // Escaped pair like \* — treat as one symbol token
      var pair = c + (re[i + 1] || '');
      out += pair;
      i += 2;
      var nx = re[i];
...
function toPostfix(re) {
  ...
  for (var i = 0; i < re.length; i++) {
    var ch = re[i];
    if (ch === '\\') {
      // Escaped literal — push the next char as a plain symbol
      out.push(re[i + 1] || '\\');
      i++;
      ...
      while (ops.length && prec[ops[ops.length-1]] !== undefined &&
        (prec[ops[ops.length-1]] > prec[ch] ||
         (prec[ops[ops.length-1]] === prec[ch] && !unary))) out.push(ops.pop());
      ops.push(ch);
    } else { out.push(ch); }
  }
  while (ops.length) { var op=ops.pop(); if(op==='(') throw new Error('Mismatched ('); out.push(op); }
  return out;
}
...
// ════════════════════════════════════════════════════════════
var SC = 0;
function ns() { return { id: SC++, trans: {} }; }
function addT(st, sym, tid) {
  if (!st.trans[sym]) st.trans[sym] = [];
  st.trans[sym].push(tid);
}
function snapFrag(f) {
  return {
    startId:  f.start.id,
    acceptId: f.accept.id,
    states:   f.ls.map(function(s){ return {id:s.id}; }),
    trans:    f.ft.map(function(t){ return {from:t.from,sym:t.sym,to:t.to}; })
  };
}
function mkF(start,accept,ls,ft){ return {start:start,accept:accept,ls:ls,ft:ft}; }

function fSym(ch) {
  var s=ns(),a=ns();
  addT(s,ch,a.id);
  return mkF(s,a,[s,a],[{from:s.id,sym:ch,to:a.id}]);
}
function fConcat(f1,f2) {
  addT(f1.accept,'ε',f2.start.id);
  return mkF(f1.start,f2.accept,f1.ls.concat(f2.ls),
    f1.ft.concat(f2.ft,[{from:f1.accept.id,sym:'ε',to:f2.start.id}]));
}
function fUnion(f1,f2) {
  var s=ns(),a=ns();
  addT(s,'ε',f1.start.id); addT(s,'ε',f2.start.id);
  addT(f1.accept,'ε',a.id); addT(f2.accept,'ε',a.id);
  var ts=[{from:s.id,sym:'ε',to:f1.start.id},{from:s.id,sym:'ε',to:f2.start.id},
          {from:f1.accept.id,sym:'ε',to:a.id},{from:f2.accept.id,sym:'ε',to:a.id}];
  return mkF(s,a,[s,a].concat(f1.ls,f2.ls),f1.ft.concat(f2.ft,ts));
}
function fKleene(f) {
  var s=ns(),a=ns();
  addT(s,'ε',f.start.id); addT(s,'ε',a.id);
  addT(f.accept,'ε',f.start.id); addT(f.accept,'ε',a.id);
  var ts=[{from:s.id,sym:'ε',to:f.start.id},{from:s.id,sym:'ε',to:a.id},
          {from:f.accept.id,sym:'ε',to:f.start.id},{from:f.accept.id,sym:'ε',to:a.id}];
  return mkF(s,a,[s,a].concat(f.ls),f.ft.concat(ts));
}
function fPlus(f) {
  var s=ns(),a=ns();
  addT(s,'ε',f.start.id); addT(f.accept,'ε',f.start.id); addT(f.accept,'ε',a.id);
  var ts=[{from:s.id,sym:'ε',to:f.start.id},
          {from:f.accept.id,sym:'ε',to:f.start.id},{from:f.accept.id,sym:'ε',to:a.id}];
  return mkF(s,a,[s,a].concat(f.ls),f.ft.concat(ts));
}
function fQuestion(f) {
  var s=ns(),a=ns();
  addT(s,'ε',f.start.id); addT(s,'ε',a.id); addT(f.accept,'ε',a.id);
  var ts=[{from:s.id,sym:'ε',to:f.start.id},{from:s.id,sym:'ε',to:a.id},
          {from:f.accept.id,sym:'ε',to:a.id}];
  return mkF(s,a,[s,a].concat(f.ls),f.ft.concat(ts));
}

function buildNFA(regex) {
  SC = 0;
  var postfix = toPostfix(insertConcat(normalizeUnion(regex.trim())));
  var stack=[], steps=[];
  // Track ALL states and transitions built so far for accumulated display
  var allStates=[], allTrans=[];
  for (var i=0;i<postfix.length;i++) {
    var tok=postfix[i], frag, rule, why, res;
    if (tok==='.') {
      var f2=stack.pop(),f1=stack.pop(); frag=fConcat(f1,f2);
      rule='Concatenation'; 
      why='Chain two fragments: an ε-transition from q'+f1.accept.id+' (accept of left) to q'+f2.start.id+' (start of right). No input is consumed at the join.';
      res='q'+f1.start.id+' ──...──▶ q'+f1.accept.id+' ──ε──▶ q'+f2.start.id+' ──...──▶ q'+f2.accept.id;
    } else if (tok==='|') {
      var f2u=stack.pop(),f1u=stack.pop(); frag=fUnion(f1u,f2u);
      rule='Union (+)';
      why='New start q'+frag.start.id+' branches via ε to both sub-NFAs. Both accept states merge via ε into new accept q'+frag.accept.id+'. Either path leads to acceptance.';
      res='q'+frag.start.id+' ──ε──▶ {q'+f1u.start.id+', q'+f2u.start.id+'} → ... → q'+frag.accept.id;
    } else if (tok==='*') {
      var fk=stack.pop(); frag=fKleene(fk);
      rule='Kleene Star (*)';
      why='New start q'+frag.start.id+' can skip to accept (zero times) or enter q'+fk.start.id+'. After matching, q'+fk.accept.id+' loops back for repetition or exits to accept q'+frag.accept.id+'.';
      res='q'+frag.start.id+' ──ε──▶ q'+fk.start.id+' (enter) | q'+frag.accept.id+' (skip). Loop: q'+fk.accept.id+' ──ε──▶ q'+fk.start.id;
    } else if (tok==='+') {
      var fp=stack.pop(); frag=fPlus(fp);
      rule='Plus (+)';
      why='Must enter q'+fp.start.id+' at least once (no skip path). After matching, q'+fp.accept.id+' can loop back to q'+fp.start.id+' or exit to accept q'+frag.accept.id+'.';
      res='q'+frag.start.id+' ──ε──▶ q'+fp.start.id+' (mandatory). q'+fp.accept.id+' loops or exits to q'+frag.accept.id;
    } else if (tok==='?') {
      var fq=stack.pop(); frag=fQuestion(fq);
      rule='Optional (?)';
      why='New start q'+frag.start.id+' either skips directly to accept q'+frag.accept.id+' (zero times) via ε, or enters q'+fq.start.id+' for exactly one match.';
      res='q'+frag.start.id+' ──ε──▶ q'+fq.start.id+' OR q'+frag.accept.id+' (skip)';
    } else {
      frag=fSym(tok); rule='Symbol "'+tok+'"';
      why='Base case: two new states q'+frag.start.id+' and q'+frag.accept.id+', connected by a single transition on "'+tok+'". This is the atomic Thompson fragment.';
      res='q'+frag.start.id+' ──"'+tok+'"──▶ q'+frag.accept.id;
    }
    stack.push(frag);
    // Accumulate ALL states and transitions seen so far
    frag.ls.forEach(function(s){ if(!allStates.find(function(x){return x.id===s.id;})) allStates.push({id:s.id}); });
    frag.ft.forEach(function(t){ if(!allTrans.find(function(x){return x.from===t.from&&x.sym===t.sym&&x.to===t.to;})) allTrans.push({from:t.from,sym:t.sym,to:t.to}); });
    // Build accumulated snapshot: all states/trans so far, highlight current fragment
    var accumSnap = {
      ...
      fragStateIds: frag.ls.map(function(s){return s.id;})
    };
    steps.push({rule:rule,why:why,res:res,snap:accumSnap});
  }
  var final=stack.pop();
  var finalSnap=final?{
    states: allStates.slice(),
    trans: allTrans.slice(),
    startId: final.start.id,
    acceptId: final.accept.id,
    fragStartId: final.start.id,
    fragAcceptId: final.accept.id,
    fragStateIds: final.ls.map(function(s){return s.id;})
  }:null;
  return {steps:steps, final: finalSnap};
}
...
// ════════════════════════════════════════════════════════════
// §4  SIMULATION (NFA via ε-closure; DFA direct)
// ════════════════════════════════════════════════════════════
function simulateNFA(nfaSnap, str) {
  ...
  for(var i=0;i<str.length;i++){
    ...
    Object.keys(cur).forEach(function(sid){
      (tm[sid]&&tm[sid][ch]||[]).forEach(function(nid){nxt[nid]=true;});
    });
...
// ── Step-tracking simulation (for trace panel) ──
function simulateNFAWithSteps(nfaSnap, str) {
  ...
  var cur=epsClosure([nfaSnap.startId],tm);
  var steps=[];
  var initIds=Object.keys(cur).map(Number).sort(function(a,b){return a-b;});
  steps.push({input:null,stateIds:initIds,
    desc:'ε-closure({q'+nfaSnap.startId+'}) = {q'+initIds.join(', q')+'}'});
  for(var i=0;i<str.length;i++){
    var ch=str[i],nxt={};
    Object.keys(cur).forEach(function(sid){
      (tm[sid]&&tm[sid][ch]||[]).forEach(function(nid){nxt[nid]=true;});
    });
    var moved=Object.keys(nxt).map(Number).sort(function(a,b){return a-b;});
    cur=epsClosure(Object.keys(nxt).map(Number),tm);
    var afterIds=Object.keys(cur).map(Number).sort(function(a,b){return a-b;});
    steps.push({input:ch,stateIds:afterIds,
      desc:moved.length?
        'move→{q'+moved.join(',q')+'} → ε-closure→{q'+afterIds.join(',q')+'}':
        'move→∅ (no transition on "'+ch+'")'});
    if(!afterIds.length){return{accepted:false,steps:steps,deadOn:ch};}
  }
  return{accepted:!!cur[nfaSnap.acceptId],steps:steps};
}
function simulateDFAWithSteps(dfaSnap, str) {
  ...
  var cur=dfaSnap.startLabel;
  var steps=[];
  steps.push({input:null,state:cur,desc:'Start state: '+cur});
  for(var i=0;i<str.length;i++){
    var ch=str[i];
    var nx=tm[cur]&&tm[cur][ch];
    if(!nx||nx==='∅'){
      steps.push({input:ch,state:'∅',desc:'δ('+cur+', "'+ch+'") = ∅ → dead state'});
      return{accepted:false,steps:steps};
    }
    steps.push({input:ch,state:nx,desc:'δ('+cur+', "'+ch+'") = '+nx+(dfaSnap.accepts[nx]?' [accept]':'')});
    cur=nx;
  }
  return{accepted:!!dfaSnap.accepts[cur],steps:steps,finalState:cur};
}

// ── Simulation Trace Component ──
function SimTrace(props) {
  var trace=props.trace, isNfa=props.isNfa;
  if(!trace) return null;
  return(
    React.createElement('div', { className: "simtrace", __self: this, __source: {fileName: _jsxFileName, lineNumber: 527}}
      , React.createElement('div', { className: "simtrace-hd", __self: this, __source: {fileName: _jsxFileName, lineNumber: 528}}, isNfa?'NFA':'DFA', " Simulation Trace — \""    , props.input, "\"")
      , trace.steps.map(function(step,i){
        return(
          React.createElement('div', { key: i, className: "simtrace-row", __self: this, __source: {fileName: _jsxFileName, lineNumber: 531}}
            , React.createElement('span', { className: "simtrace-idx", __self: this, __source: {fileName: _jsxFileName, lineNumber: 532}}, i===0?'▶':(i+'.'))
            , step.input!==null&&React.createElement('span', { className: "simtrace-ch", __self: this, __source: {fileName: _jsxFileName, lineNumber: 533}}, "\"", step.input, "\"")
            , step.input===null&&React.createElement('span', { className: "simtrace-ch", style: {background:'rgba(91,140,255,.1)',color:'var(--muted)'}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 534}}, "init")
            , React.createElement('span', { className: "simtrace-desc", __self: this, __source: {fileName: _jsxFileName, lineNumber: 535}}
              , step.desc
              , isNfa&&step.stateIds!==undefined&&step.stateIds.length>0&&(
                React.createElement('span', { className: "simtrace-states", __self: this, __source: {fileName: _jsxFileName, lineNumber: 538}}, " → active: "   , '{q'+step.stateIds.join(',q')+'}')
              )
            )
          )
        );
      })
      , React.createElement('div', { className: "simtrace-verdict", __self: this, __source: {fileName: _jsxFileName, lineNumber: 544}}
        , trace.accepted?
          React.createElement(React.Fragment, null, React.createElement('span', { style: {color:'var(--a3)'}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 546}}, "✓"), React.createElement('span', { style: {color:'var(--a3)'}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 546}}, "ACCEPTED — final state "    , isNfa?'contains NFA accept state':'is an accept state')):
          React.createElement(React.Fragment, null, React.createElement('span', { style: {color:'var(--a2)'}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 547}}, "✗"), React.createElement('span', { style: {color:'var(--a2)'}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 547}}, "REJECTED — "  , isNfa?'no accept state in active set':'not in an accept state'))
...
// ════════════════════════════════════════════════════════════
// §6  SVG VISUALIZATION COMPONENTS
// ════════════════════════════════════════════════════════════
function SvgMarkers() {
  return (
    React.createElement('defs', {__self: this, __source: {fileName: _jsxFileName, lineNumber: 631}}
      , React.createElement('marker', { id: "ae", markerWidth: "9", markerHeight: "6", refX: "8", refY: "3", orient: "auto", __self: this, __source: {fileName: _jsxFileName, lineNumber: 632}}
        , React.createElement('polygon', { points: "0 0,9 3,0 6"   , fill: "#f7971e", __self: this, __source: {fileName: _jsxFileName, lineNumber: 633}})
      )
      , React.createElement('marker', { id: "as", markerWidth: "9", markerHeight: "6", refX: "8", refY: "3", orient: "auto", __self: this, __source: {fileName: _jsxFileName, lineNumber: 635}}
        , React.createElement('polygon', { points: "0 0,9 3,0 6"   , fill: "#5b8cff", __self: this, __source: {fileName: _jsxFileName, lineNumber: 636}})
      )
      , React.createElement('marker', { id: "ap", markerWidth: "9", markerHeight: "6", refX: "8", refY: "3", orient: "auto", __self: this, __source: {fileName: _jsxFileName, lineNumber: 638}}
        , React.createElement('polygon', { points: "0 0,9 3,0 6"   , fill: "#9b59b6", __self: this, __source: {fileName: _jsxFileName, lineNumber: 639}})
      )
      , React.createElement('marker', { id: "adfa", markerWidth: "9", markerHeight: "6", refX: "8", refY: "3", orient: "auto", __self: this, __source: {fileName: _jsxFileName, lineNumber: 641}}
        , React.createElement('polygon', { points: "0 0,9 3,0 6"   , fill: "#c77dff", __self: this, __source: {fileName: _jsxFileName, lineNumber: 642}})
      )
      , React.createElement('marker', { id: "adead", markerWidth: "9", markerHeight: "6", refX: "8", refY: "3", orient: "auto", __self: this, __source: {fileName: _jsxFileName, lineNumber: 643}}
        , React.createElement('polygon', { points: "0 0,9 3,0 6"   , fill: "#ff6b6b", __self: this, __source: {fileName: _jsxFileName, lineNumber: 644}})
      )
...
function distPointToSeg(px,py,ax,ay,bx,by){
  ...
  if(l2<1e-6) return Math.sqrt((px-ax)*(px-ax)+(py-ay)*(py-ay));
  var t=Math.max(0,Math.min(1,((px-ax)*dx+(py-ay)*dy)/l2));
  var cx2=ax+t*dx-px, cy2=ay+t*dy-py;
  return Math.sqrt(cx2*cx2+cy2*cy2);
}

function buildEdges(states, trans, getPos, R, isDfa) {
  // Build allPos for obstacle avoidance
  var allPos={};
  states.forEach(function(s){ var p=getPos(s.id); if(p) allPos[String(s.id)]=p; });

  // Group by from-to, collect labels
  var edgeMap={}, seen={};
  trans.forEach(function(t){
    var k=t.from+'-'+t.to;
    if(!edgeMap[k]) edgeMap[k]=[];
    if(edgeMap[k].indexOf(t.sym)===-1) edgeMap[k].push(t.sym);
  });
  var edges=[];
  trans.forEach(function(t){
    var k=t.from+'-'+t.to, rev=t.to+'-'+t.from;
    if(seen[k]) return; seen[k]=true;
    var p1=getPos(t.from), p2=getPos(t.to);
    if(!p1||!p2) return;
    var labels=edgeMap[k];
    var allEps=!isDfa&&labels.every(function(l){return l==='ε';});
    var color=isDfa?'#c77dff':(allEps?'#f7971e':'#5b8cff');
    var marker=isDfa?'url(#adfa)':(allEps?'url(#ae)':'url(#as)');
    var isSelf=t.from===t.to, hasBoth=!!(edgeMap[rev]&&edgeMap[rev].length);
    var d,lx,ly;
    if(isSelf){
      var cxs=p1.x, cys=p1.y-95;
      d='M '+p1.x+','+(p1.y-R)+' C '+(cxs-56)+','+(cys-46)+' '+(cxs+56)+','+(cys-46)+' '+p1.x+','+(p1.y-R);
      lx=cxs; ly=cys-50;
    } else {
      var dx=p2.x-p1.x, dy=p2.y-p1.y, len=Math.sqrt(dx*dx+dy*dy)||1;
      var ux=dx/len, uy=dy/len;
      var sx=p1.x+ux*R, sy=p1.y+uy*R, ex=p2.x-ux*R, ey=p2.y-uy*R;
      // perpendicular unit
      var px2=-uy, py2=ux;
      if(hasBoth){
        // bidirectional: always curve to one side
        var off=58;
        var qx=(sx+ex)/2+px2*off, qy=(sy+ey)/2+py2*off;
        d='M '+sx+','+sy+' Q '+qx+','+qy+' '+ex+','+ey;
        // label at curve midpoint, offset outward
        lx=0.25*sx+0.5*qx+0.25*ex+px2*16;
        ly=0.25*sy+0.5*qy+0.25*ey+py2*16;
      } else {
        // straight line — check for obstacles
        var blocked=null, bDist=999;
        Object.keys(allPos).forEach(function(id){
          if(id===String(t.from)||id===String(t.to)) return;
          var np=allPos[id];
          var dist=distPointToSeg(np.x,np.y,sx,sy,ex,ey);
          if(dist < R+16 && dist < bDist){ bDist=dist; blocked=np; }
        });
        if(blocked){
          // determine which side to curve to avoid the blocking node
          var mx=(sx+ex)/2, my=(sy+ey)/2;
          var dot2=(blocked.x-mx)*px2+(blocked.y-my)*py2;
          var side=dot2>0?-1:1;
          var curveMag=R+60+Math.max(0,(R+16-bDist)*1.8);
          var qx2=mx+px2*side*curveMag, qy2=my+py2*side*curveMag;
          d='M '+sx+','+sy+' Q '+qx2+','+qy2+' '+ex+','+ey;
          lx=0.25*sx+0.5*qx2+0.25*ex+px2*side*18;
          ly=0.25*sy+0.5*qy2+0.25*ey+py2*side*18;
        } else {
          d='M '+sx+','+sy+' L '+ex+','+ey;
          lx=(sx+ex)/2+px2*22; ly=(sy+ey)/2+py2*22;
        }
      }
    }
    edges.push({k:k,d:d,lx:lx,ly:ly,labels:labels,color:color,marker:marker,allEps:allEps});
  });
  return edges;
}
...
function ZoomBar(props){
  var zoom=props.zoom,zoomIn=props.zoomIn,zoomOut=props.zoomOut,fit=props.fit;
  var btnS={background:'var(--hl)',border:'1px solid var(--border)',color:'var(--text)',
    borderRadius:5,cursor:'pointer',fontFamily:'Syne,sans-serif',fontWeight:700,
    fontSize:'.75rem',padding:'.2rem .5rem',lineHeight:1,transition:'all .13s'};
  return React.createElement('div',{style:{
    position:'absolute',top:8,right:8,zIndex:50,display:'flex',alignItems:'center',gap:4,
    background:'rgba(10,12,16,.88)',border:'1px solid var(--border)',borderRadius:7,
    padding:'4px 7px',backdropFilter:'blur(6px)',userSelect:'none'
  }},
    React.createElement('button',{style:btnS,onClick:zoomOut},'−'),
    React.createElement('span',{style:{fontFamily:'JetBrains Mono,monospace',fontSize:'.62rem',
      color:'var(--muted)',minWidth:34,textAlign:'center'}},Math.round(zoom*100)+'%'),
    React.createElement('button',{style:btnS,onClick:zoomIn},'+'),
    React.createElement('button',{style:Object.assign({},btnS,{fontSize:'.58rem',color:'var(--muted)',marginLeft:2}),onClick:fit},'Fit')
  );
}
...
function NFAViz(props) {
  ...
  if(!snap||!snap.states||!snap.states.length) {
    return (
      React.createElement('div', { ref: cref, style: {position:'absolute',inset:0,display:'flex',flexDirection:'column',
  ...
  function onMouseMove(e){
    ...
    setVb({w:dragging.current.vb.w,h:dragging.current.vb.h,
           x:dragging.current.vb.x-dx,y:dragging.current.vb.y-dy});
  }
  function onMouseUp(){dragging.current=null;}

  return (
    React.createElement('div', { ref: cref, style: {position:'absolute',inset:0,overflow:'hidden',cursor:'grab'},
      onMouseDown:onMouseDown,onMouseMove:onMouseMove,onMouseUp:onMouseUp,onMouseLeave:onMouseUp}
      , React.createElement(ZoomBar,{zoom:zoomPct,zoomIn:zoomIn,zoomOut:zoomOut,fit:fit})
      , React.createElement('svg', { width: cW, height: cH,
        ...
        , edges.map(function(e){
          ...
          return(
            React.createElement('g', { key: e.k, style: {opacity:edgeDim?.3:edgePrevOnly?.25:1,transition:'opacity .3s'}}
              , React.createElement('path', { d: e.d, fill: "none", stroke: e.color, strokeWidth: "1.65",
              ...
              , React.createElement('text', { x: e.lx, y: e.ly, fill: e.color, textAnchor: "middle",
                fontSize: "10", fontFamily: "JetBrains Mono,monospace", fontWeight: "700"}, labelStr)
            )
          );
        })
        , snap.states.map(function(s){
          ...
          return(
            React.createElement('g', { key: s.id, style: {opacity:dim?.25:1,transition:'opacity .3s'}}
              ...
              , React.createElement('circle', { cx: p.x, cy: p.y, r: R, fill: fill, stroke: stroke, strokeWidth: sw,
                style: {filter:'drop-shadow(0 0 '+(isActive?'9px':'5px')+' '+stroke+'60)',transition:'all .3s'}})
              , React.createElement('text', { x: p.x, y: p.y+4, textAnchor: "middle", fill: isActive?'#fff':"#e2e8f8", fontSize: "9.5",
                fontFamily: "JetBrains Mono,monospace", fontWeight: "600"}, "q", s.id)
              , (isAccept||isFinalActive)&&stateLabel&&(React.createElement('text', { x: p.x, y: p.y+R+15, textAnchor: "middle", fill: stateLabelColor,
                fontSize: "7", fontFamily: "JetBrains Mono,monospace"}, stateLabel))
            )
          );
        })
      )
  ...
  return [
    edges.map(function(e){
      ...
      return React.createElement('g',{key:e.k,style:{transition:'opacity .3s',opacity:isDeadEdge?.55:1}},
        React.createElement('path',{d:e.d,fill:'none',stroke:isDeadEdge?'#ff6b6b':e.color,
        ...
        React.createElement('rect',{x:e.lx-labelW/2,y:e.ly-9,width:labelW,height:13,rx:3,fill:'var(--bg)',opacity:.85}),
        React.createElement('text',{x:e.lx,y:e.ly,fill:isDeadEdge?'#ff6b6b':e.color,textAnchor:'middle',
          fontSize:'10',fontFamily:'JetBrains Mono,monospace',fontWeight:'700'},labelStr)
      );
    }),
    snap.states.map(function(s){
      ...
      if(isDead) return React.createElement('g',{key:s.label,style:{opacity:.75}},
        React.createElement('circle',{cx:p.x,cy:p.y,r:R,fill:'#1a0808',stroke:'#ff6b6b',strokeWidth:'1.5',
          strokeDasharray:'4,2',style:{filter:'drop-shadow(0 0 5px #ff6b6b44)'}}),
        React.createElement('text',{x:p.x,y:p.y+5,textAnchor:'middle',fill:'#ff6b6b',fontSize:'14',
          fontFamily:'JetBrains Mono,monospace',fontWeight:'700'},'∅'),
        React.createElement('text',{x:p.x,y:p.y+R+14,textAnchor:'middle',fill:'#ff6b6b',
          fontSize:'7',fontFamily:'JetBrains Mono,monospace'},'DEAD')
      );
      ...
      return React.createElement('g',{key:s.label,style:{opacity:dim?.25:1,transition:'opacity .3s'}},
        isStart&&React.createElement('g',null,
          React.createElement('line',{x1:p.x-R-26,y1:p.y,x2:p.x-R-1,y2:p.y,
            stroke:isFinalActive?finalColor:(isActive?'#f7971e':accentColor),strokeWidth:'1.5',markerEnd:markerUrl}),
          React.createElement('text',{x:p.x-R-28,y:p.y-7,fill:isFinalActive?finalColor:(isActive?'#f7971e':accentColor),
            fontSize:'7.5',textAnchor:'end',fontFamily:'JetBrains Mono,monospace'},'START')
        ),
        isAccept&&React.createElement('circle',{cx:p.x,cy:p.y,r:R+5,fill:'none',
        ...
        React.createElement('text',{x:p.x,y:p.y+1,textAnchor:'middle',fill:isActive?'#fff':'#e2e8f8',
          fontSize:'11',fontFamily:'JetBrains Mono,monospace',fontWeight:'700'},s.label),
        (isAccept||isFinalActive)&&stateLabel&&React.createElement('text',{x:p.x,y:p.y+R+15,textAnchor:'middle',
          fill:isFinalActive?finalColor:'#43e97b',fontSize:'7',fontFamily:'JetBrains Mono,monospace'},stateLabel),
        React.createElement('text',{x:p.x,y:p.y+R+26,textAnchor:'middle',fill:'var(--muted)',
          fontSize:'7',fontFamily:'JetBrains Mono,monospace'},nfaLabel)
      );
    })
  ];
}

// Shared viewBox zoom+pan logic factory
function useViewZoom(dep) {
  ...
  useEffect(function(){setVb(null);},[dep]);

  return {cref:cref, dims:dims, vb:vb, setVb:setVb, dragging:dragging};
}
...
function DFAViz(props) {
  ...
  if(!dfaSnap||!dfaSnap.states||!dfaSnap.states.length){
    return React.createElement('div',{ref:cref,style:{position:'absolute',inset:0,display:'flex',flexDirection:'column',
  ...
  return React.createElement('div',{ref:cref,style:{position:'absolute',inset:0,overflow:'hidden',cursor:'grab'},
    onMouseDown:onMouseDown,onMouseMove:onMouseMove,onMouseUp:onMouseUp,onMouseLeave:onMouseUp},
    React.createElement(ZoomBar,{zoom:zoomPct,zoomIn:zoomIn,zoomOut:zoomOut,fit:fit}),
    React.createElement('svg',{width:dims.w,height:dims.h,viewBox:cvb.x+' '+cvb.y+' '+cvb.w+' '+cvb.h,
...
function MinDFAViz(props) {
  ...
  if(!dfaSnap||!dfaSnap.states||!dfaSnap.states.length){
    return React.createElement('div',{ref:cref,style:{position:'absolute',inset:0,display:'flex',flexDirection:'column',
      alignItems:'center',justifyContent:'center',gap:'.9rem',color:'var(--muted)',textAlign:'center',pointerEvents:'none'}},
      React.createElement('div',{style:{fontSize:'2.4rem',opacity:.2}},'◈'),
      React.createElement('div',{style:{fontSize:'.77rem',maxWidth:'200px',lineHeight:1.75}},'Build NFA → DFA first, then press ',
        React.createElement('strong',{style:{color:'var(--a4)'}},'Minimize DFA'))
    );
  }
  ...
  var allStateIds=dfaSnap.states.map(function(s){return {id:s.label};});
  var edges=buildEdges(allStateIds,dfaSnap.trans.map(function(t){return {from:t.from,sym:t.sym,to:t.to};}),getPos,R,true);
  // Override edge color for min-DFA to orange
  edges.forEach(function(e){if(e.k.indexOf('∅')===-1){e.color='#f7971e';e.marker='url(#amin)';}});
  ...
  return React.createElement('div',{ref:cref,style:{position:'absolute',inset:0,overflow:'hidden',cursor:'grab'},
    ...
    React.createElement('svg',{width:dims.w,height:dims.h,viewBox:cvb.x+' '+cvb.y+' '+cvb.w+' '+cvb.h,
      style:{position:'absolute',inset:0},onWheel:onWheel},
      React.createElement(SvgMarkers),
      renderDFAContent(dfaSnap,pos,R,edges,activeLabel!==null,activeLabel,simResult,'#f7971e','url(#amin)')
    )
  );
}
...
function App() {
  ...
  var ss=useState([]);          var nfaSteps=ss[0],setNfaSteps=ss[1];
  var ni=useState(-1);          var nfaIdx=ni[0],setNfaIdx=ni[1];
  var fn=useState(null);        var finalNfa=fn[0],setFinalNfa=fn[1];
  var ds=useState(null);        var dfaData=ds[0],setDfaData=ds[1];    // {steps,dfaSnap,alphabet}
  var di=useState(-1);          var dfaIdx=di[0],setDfaIdx=di[1];
  var es=useState('');          var err=es[0],setErr=es[1];
  var vi=useState('nfa');       var viewTab=vi[0],setViewTab=vi[1];    // 'nfa'|'dfa'|'min'
  var sim=useState('');         var simIn=sim[0],setSimIn=sim[1];
  var sr=useState(null);        var simRes=sr[0],setSimRes=sr[1];
  var th=useState(false);       var showTh=th[0],setShowTh=th[1];
  var dm=useState(true);        var darkMode=dm[0],setDarkMode=dm[1];
  var st=useState(null);        var simTrace=st[0],setSimTrace=st[1];
  var sp=useState(-1);          var simStep=sp[0],setSimStep=sp[1];  // -1 = not stepped
  var md=useState(null);        var minDfaData=md[0],setMinDfaData=md[1]; // {steps,minSnap,origCount,minCount}
  var mx=useState(-1);          var minDfaIdx=mx[0],setMinDfaIdx=mx[1];

  // auto-play state
  var _ap=useState(false); var isPlaying=_ap[0],setIsPlaying=_ap[1];
  var _sp2=useState(false); var simPlaying=_sp2[0],setSimPlaying=_sp2[1];
  var playRef=useRef(null);
  var simPlayRef=useRef(null);
  var slistRef=useRef(null);
  var activeCardRef=useRef(null);
  var rxiRef=useRef(null);
  ...
  useEffect(function(){
    if(activeCardRef.current){activeCardRef.current.scrollIntoView({behavior:'smooth',block:'nearest'});}
  },[nfaIdx,dfaIdx,minDfaIdx]);
  ...
  useEffect(function(){
    ...
    playRef.current=setInterval(function(){
      if(viewTab==='nfa'){setNfaIdx(function(i){if(i>=nfaSteps.length){setIsPlaying(false);return i;}return i+1;});}
      else if(viewTab==='dfa'){setDfaIdx(function(i){if(!dfaData||i>=dfaData.steps.length-1){setIsPlaying(false);return i;}return i+1;});}
      else if(viewTab==='min'){setMinDfaIdx(function(i){if(!minDfaData||i>=minDfaData.steps.length-1){setIsPlaying(false);return i;}return i+1;});}
    },1100);
    return function(){clearInterval(playRef.current);playRef.current=null;};
  },[isPlaying,viewTab,nfaSteps,dfaData,minDfaData]);

  // sim auto-play
  useEffect(function(){
    if(!simPlaying){if(simPlayRef.current){clearInterval(simPlayRef.current);simPlayRef.current=null;}return;}
    simPlayRef.current=setInterval(function(){
      setSimStep(function(s){if(!simTrace||s>=simTrace.steps.length-1){setSimPlaying(false);return s;}return s+1;});
    },900);
    return function(){clearInterval(simPlayRef.current);simPlayRef.current=null;};
  },[simPlaying,simTrace]);

  // ── keyboard shortcuts (Enter=play/pause, ←/→=prev/next) ──
  var kbState=useRef({viewTab:'nfa',nfaLen:0,dfaLen:0,minLen:0});
  useEffect(function(){
    kbState.current={viewTab:viewTab,nfaLen:nfaSteps.length,dfaLen:dfaData?dfaData.steps.length:0,minLen:minDfaData?minDfaData.steps.length:0};
  });
  useEffect(function(){
    function handleKey(e){
      /* Alt+E inserts ε into the regex input from anywhere */
      if(e.altKey && (e.key==='e'||e.key==='E')){
        e.preventDefault();
        insertAtCursor(rxiRef,'ε',setRegex);
        return;
      }
      if(e.target.tagName==='INPUT'||e.target.tagName==='TEXTAREA') return;
      var kb=kbState.current;
      if(e.key==='Enter'){
        e.preventDefault();
        ...
        e.preventDefault();
        setIsPlaying(false);
        if(kb.viewTab==='nfa') setNfaIdx(function(i){return Math.max(0,i-1);});
        else if(kb.viewTab==='dfa') setDfaIdx(function(i){return Math.max(0,i-1);});
        else if(kb.viewTab==='min') setMinDfaIdx(function(i){return Math.max(0,i-1);});
      } else if(e.key==='ArrowRight'){
        e.preventDefault();
        setIsPlaying(false);
        if(kb.viewTab==='nfa') setNfaIdx(function(i){return Math.min(kb.nfaLen,i+1);});
        else if(kb.viewTab==='dfa') setDfaIdx(function(i){return Math.min(Math.max(kb.dfaLen-1,0),i+1);});
        else if(kb.viewTab==='min') setMinDfaIdx(function(i){return Math.min(Math.max(kb.minLen-1,0),i+1);});
      }
    }
  ...
  function handleBuild(){
    setErr(''); setDfaData(null); setDfaIdx(-1); setSimRes(null); setMinDfaData(null); setMinDfaIdx(-1);
    if(!regex.trim()){setErr('Please enter a regular expression.');return;}
    try{
      var r=buildNFA(regex.trim());
      setNfaSteps(r.steps); setFinalNfa(r.final); setNfaIdx(0);
      setViewTab('nfa');
      setTimeout(function(){setIsPlaying(true);},80);
    }catch(e){setErr('Error: '+e.message);}
  }

  // ── convert to DFA ──
  function handleDFA(){
    if(!finalNfa){setErr('Build the NFA first.');return;}
    setErr(''); setSimRes(null); setMinDfaData(null); setMinDfaIdx(-1);
    setIsPlaying(false);
    try{
      var d=subsetConstruct(finalNfa);
      setDfaData(d); setDfaIdx(0); setViewTab('dfa');
      setTimeout(function(){setIsPlaying(true);},80);
    }catch(e){setErr('DFA error: '+e.message);}
  }

  // ── minimize DFA ──
  function handleMinimize(){
    if(!dfaData){setErr('Build the DFA first.');return;}
    setErr(''); setSimRes(null);
    setIsPlaying(false);
    try{
      var m=minimizeDFA(dfaData.dfaSnap, dfaData.alphabet);
      setMinDfaData(m); setMinDfaIdx(0); setViewTab('min');
      setTimeout(function(){setIsPlaying(true);},80);
    }catch(e){setErr('Minimize error: '+e.message);}
  }

  function handleClear(){
    setRegex(''); setNfaSteps([]); setFinalNfa(null);
    setDfaData(null); setNfaIdx(-1); setDfaIdx(-1);
    setMinDfaData(null); setMinDfaIdx(-1);
    setErr(''); setSimRes(null); setSimIn(''); setSimTrace(null); setSimStep(-1);
    setIsPlaying(false); setSimPlaying(false);
  }

  // ── simulation ──
  function handleSim(){
    var trace=null;
    if(viewTab==='min'&&minDfaData){
      trace=simulateDFAWithSteps(minDfaData.minSnap,simIn);
    } else if(viewTab==='dfa'&&dfaData){
      trace=simulateDFAWithSteps(dfaData.dfaSnap,simIn);
    } else if(finalNfa){
      trace=simulateNFAWithSteps(finalNfa,simIn);
    }
    if(trace){setSimRes(trace.accepted);setSimTrace(trace);setSimStep(0);setTimeout(function(){setSimPlaying(true);},80);}
  }

  // ── active states for diagram highlighting ──
  var simActiveNfaIds=null, simActiveDfaLabel=null, simActiveMinLabel=null;
  // Is this the last step of simulation?
  var isLastSimStep = simTrace && simStep === simTrace.steps.length - 1;
  var simFinalResult = isLastSimStep ? simRes : null; // only color on last step
  if(simTrace&&simStep>=0&&simStep<simTrace.steps.length){
    var curSimStep=simTrace.steps[simStep];
    if(viewTab==='nfa'){
      var rawIds = curSimStep.stateIds||null;
      if(rawIds && rawIds.length > 0){
        simActiveNfaIds = rawIds;
      } else if((!rawIds || rawIds.length===0) && simStep>0){
        // No active states (dead): highlight previous step's states in red
        simActiveNfaIds = simTrace.steps[simStep-1].stateIds||null;
      }
    } else if(viewTab==='min'){
      var rawMinLabel = curSimStep.state;
      if(rawMinLabel && rawMinLabel!=='∅'){
        simActiveMinLabel = rawMinLabel;
      } else if(rawMinLabel==='∅' && simStep>0){
        // Dead state: highlight the previous real state in red
        simActiveMinLabel = simTrace.steps[simStep-1].state||null;
      }
    } else {
      var rawDfaLabel = curSimStep.state;
      if(rawDfaLabel && rawDfaLabel!=='∅'){
        simActiveDfaLabel = rawDfaLabel;
      } else if(rawDfaLabel==='∅' && simStep>0){
        simActiveDfaLabel = simTrace.steps[simStep-1].state||null;
      }
    }
  }
  ...
  // ── what to show in viz ──
  var nfaDisplay=null;
  if(nfaIdx===nfaSteps.length&&finalNfa) nfaDisplay=finalNfa;
  else if(nfaIdx>=0&&nfaIdx<nfaSteps.length) nfaDisplay=nfaSteps[nfaIdx].snap;

  var dfaStepData=null, dfaDisplay=null;
  if(dfaData){
    if(dfaIdx>=0&&dfaIdx<dfaData.steps.length){
      dfaStepData=dfaData.steps[dfaIdx];
      dfaDisplay=dfaStepData.partialDfa;
    }
  }

  // Minimized DFA display: always show the full minSnap
  var minDisplay = minDfaData ? minDfaData.minSnap : null;

  var isNfaTab=viewTab==='nfa';
  var isDfaTab=viewTab==='dfa';
  var isMinTab=viewTab==='min';

  return(
    React.createElement('div', {__self: this, __source: {fileName: _jsxFileName, lineNumber: 1306}}
      /* HEADER */
      , React.createElement('header', { className: "hdr", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1308}}
        , React.createElement('div', { className: "logo", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1309}}
          , React.createElement('div', { className: "lico", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1310}}, "λ")
          , React.createElement('div', {__self: this, __source: {fileName: _jsxFileName, lineNumber: 1311}}
            , React.createElement('div', { className: "ltxt", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1312}}, "RegexAuto")
            , React.createElement('div', { className: "lsub", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1313}}, "Thompson ε-NFA + Subset DFA + Minimization"      )
          )
        )
        , React.createElement('div', { className: "hbtns", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1316}}
          , React.createElement('button', { className: "thmbtn", onClick: function(){setDarkMode(function(d){return !d;});}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1317}}
            , darkMode?'☀':'🌙'
          )
          , React.createElement('button', { className: "hbtn on" , __self: this, __source: {fileName: _jsxFileName, lineNumber: 1320}}, "Builder")
          , React.createElement('button', { className: "hbtn", onClick: function(){setShowTh(true);}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1321}}, "Theory")
        )
      )
      ...
      , React.createElement('div', { className: "lay", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1342}}
        ...
        , React.createElement('div', { className: "left", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1345}}
          , React.createElement('div', { className: "tbar", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1346}}
            ...
            , React.createElement('div', { className: "brow", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1353}}
              ...
              , React.createElement('button', { className: "btn bdfa" , onClick: handleDFA,
                disabled: !finalNfa, style: {opacity:finalNfa?1:.5}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1355}}, "⬡ → DFA"  )
              , React.createElement('button', { className: "btn", onClick: handleMinimize,
                disabled: !dfaData,
                style: {background:'var(--a4)',color:'#fff',opacity:dfaData?1:.5,flex:1}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1357}}, "◈ Min" )
              , React.createElement('button', { className: "btn bs" , onClick: handleClear,
          ...
          , React.createElement('div', { className: "slist", ref: slistRef, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1374}}
            ...
            , isNfaTab&&(
              React.createElement('div', {__self: this, __source: {fileName: _jsxFileName, lineNumber: 1378}}
                ...
                , nfaSteps.length>0&&(
                  React.createElement('div', { ref: nfaIdx===nfaSteps.length?activeCardRef:null, className: 'scard fin'+(nfaIdx===nfaSteps.length?' act':''),
            ...
            , isDfaTab&&(
              React.createElement('div', {__self: this, __source: {fileName: _jsxFileName, lineNumber: 1419}}
                ...
                , dfaData&&dfaData.steps.map(function(step,i){
                  ...
                  return(
                    React.createElement('div', { key: i, ref: act?activeCardRef:null, className: 'scard dfa-act'+(act?' act':'')+(isFin?' dfa-fin':''),
                      ...
                      , act&&(
                        React.createElement('div', {__self: this, __source: {fileName: _jsxFileName, lineNumber: 1439}}
                          ...
                          , React.createElement(DFATable, { rows: step.tableRows, alpha: dfaData.alphabet,
                ...
                , minDfaData&&(
                  React.createElement('div', {__self: this, __source: {fileName: _jsxFileName, lineNumber: 1465}}
                    ...
                    , minDfaData.steps.map(function(step,i){
                      ...
                      return(
                        React.createElement('div', { key: i, ref: act?activeCardRef:null, className: 'scard'+(act?' act':''),
          ...
          /* ── NAV ── */
          , isNfaTab&&nfaSteps.length>0&&(
            React.createElement('div', { className: "snav", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1520}}
              , React.createElement('button', { className: "nb", disabled: nfaIdx<=0,
                onClick: function(){setIsPlaying(false);setNfaIdx(function(i){return i-1;});}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1521}}, "← Prev" )
              , React.createElement('button', { className: "nb", style:{borderColor:isPlaying?'var(--a2)':'var(--a3)',color:isPlaying?'var(--a2)':'var(--a3)',minWidth:'60px'},
                onClick: function(){setIsPlaying(function(p){return !p;});}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1522}}, isPlaying?'⏸ Pause':'▶ Play')
              , React.createElement('span', { className: "sc", style:{display:'flex',flexDirection:'column',alignItems:'center',gap:'1px'}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1523}},
                React.createElement('span', null, "Step " , React.createElement('span', {__self: this, __source: {fileName: _jsxFileName, lineNumber: 1523}}, nfaIdx+1), " / "  , nfaSteps.length+1),
                React.createElement('span', {style:{fontSize:'.52rem',color:'var(--muted)',letterSpacing:'0'}}, "Enter=play/pause  ←→=step")
              )
              , React.createElement('button', { className: "nb", disabled: nfaIdx>=nfaSteps.length,
                onClick: function(){setIsPlaying(false);setNfaIdx(function(i){return i+1;});}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1524}}, "Next →" )
            )
          )
          , isDfaTab&&dfaData&&dfaData.steps.length>0&&(
            React.createElement('div', { className: "snav", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1529}}
              , React.createElement('button', { className: "nb dfa" , disabled: dfaIdx<=0,
                onClick: function(){setIsPlaying(false);setDfaIdx(function(i){return i-1;});}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1530}}, "← Prev" )
              , React.createElement('button', { className: "nb", style:{borderColor:isPlaying?'var(--a2)':'var(--a3)',color:isPlaying?'var(--a2)':'var(--a3)',minWidth:'60px'},
                onClick: function(){setIsPlaying(function(p){return !p;});}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1531}}, isPlaying?'⏸ Pause':'▶ Play')
              , React.createElement('span', { className: "sc dfa", style:{display:'flex',flexDirection:'column',alignItems:'center',gap:'1px'}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1532}},
                React.createElement('span', null, "Step " , React.createElement('span', {__self: this, __source: {fileName: _jsxFileName, lineNumber: 1532}}, dfaIdx+1), " / "  , dfaData.steps.length),
                React.createElement('span', {style:{fontSize:'.52rem',color:'var(--muted)',letterSpacing:'0'}}, "Enter=play/pause  ←→=step")
              )
              , React.createElement('button', { className: "nb dfa" , disabled: dfaIdx>=dfaData.steps.length-1,
                onClick: function(){setIsPlaying(false);setDfaIdx(function(i){return i+1;});}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1533}}, "Next →" )
            )
          )
          , isMinTab&&minDfaData&&minDfaData.steps.length>0&&(
            React.createElement('div', { className: "snav", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1538}}
              , React.createElement('button', { className: "nb", style: {borderColor:'var(--a4)',color:'var(--a4)'}, disabled: minDfaIdx<=0,
                onClick: function(){setIsPlaying(false);setMinDfaIdx(function(i){return i-1;});}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1539}}, "← Prev" )
              , React.createElement('button', { className: "nb", style:{borderColor:isPlaying?'var(--a2)':'var(--a3)',color:isPlaying?'var(--a2)':'var(--a3)',minWidth:'60px'},
                onClick: function(){setIsPlaying(function(p){return !p;});}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1540}}, isPlaying?'⏸ Pause':'▶ Play')
              , React.createElement('span', { className: "sc", style:{display:'flex',flexDirection:'column',alignItems:'center',gap:'1px'}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1541}},
                React.createElement('span', null, React.createElement('span', { style: {color:'var(--a4)'}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1541}}, minDfaIdx+1), " / "  , minDfaData.steps.length),
                React.createElement('span', {style:{fontSize:'.52rem',color:'var(--muted)',letterSpacing:'0'}}, "Enter=play/pause  ←→=step")
              )
              , React.createElement('button', { className: "nb", style: {borderColor:'var(--a4)',color:'var(--a4)'}, disabled: minDfaIdx>=minDfaData.steps.length-1,
                onClick: function(){setIsPlaying(false);setMinDfaIdx(function(i){return i+1;});}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1542}}, "Next →" )
            )
          )
        )
        ...
        , React.createElement('div', { className: "right", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1549}}
          ...
          , React.createElement('div', { className: "simbar", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1586}}
            ...
            , React.createElement('input', { className: "siminp", placeholder: "type a string to test..."    ,
              value: simIn,
          ...
          , simTrace&&simTrace.steps.length>0&&(
            React.createElement('div', { className: "simstep", __self: this, __source: {fileName: _jsxFileName, lineNumber: 1601}}
              ...
              , React.createElement('button', { className: "simnb", disabled: simStep<=0,
                onClick: function(){setSimPlaying(false);setSimStep(function(s){return s-1;});}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1603}}, "← Prev" )
              ...
              , React.createElement('button', { className: "simnb", disabled: simStep>=simTrace.steps.length-1,
                onClick: function(){setSimPlaying(false);setSimStep(function(s){return s+1;});}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1614}}, "Next →" )
              /* Final step verdict badge */
              , isLastSimStep&&(
                React.createElement('span', { className: "sr", style: {
      ...
      , showTh&&React.createElement(TheoryModal, { onClose: function(){setShowTh(false);}, __self: this, __source: {fileName: _jsxFileName, lineNumber: 1641}})
    )
  );
}
